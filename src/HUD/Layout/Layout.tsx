...
export default class Layout extends React.Component<Props, State> {
  ...
  render() {
    ...
    const leftPlayersAlive = leftPlayers.filter(player => player.state.health > 0);
    const rightPlayersAlive = rightPlayers.filter(player => player.state.health > 0);
    const EquipmentFull = rightPlayersAlive.map(player => player.state.equip_value).reduce((pre, now) => pre + now, 0) + leftPlayersAlive.map(player => player.state.equip_value).reduce((pre, now) => pre + now, 0)
    const isFreezetime = (game.round && game.round.phase === "freezetime") || game.phase_countdowns.phase === "freezetime";
    const isUtilityLevel = (game.round && game.round.phase === "freezetime") || game.phase_countdowns.phase === "bomb";
    const isFirepower = (game.round && game.round.phase === "freezetime") || game.phase_countdowns.phase === "live" || game.phase_countdowns.phase === "over" || game.phase_countdowns.phase === "warmup";
    ...
    return (
      <div className="layout">
		...
		<MVP isFreezetime={isFreezetime} veto={this.getVeto()} round={game.map.round+1}/>
        ...
        <SideBox side="left" hide={forceHide} />  
          <MoneyBox
            ...
            show={isFreezetime}
          />
          <UtilityLevel side={left.side} players={game.players} show={isUtilityLevel} />
        </div>
        ...
        <SideBox side="right" hide={forceHide} />  
          <MoneyBox 
            ...
            show={isFreezetime}
          />  
          <UtilityLevel side={right.side} players={game.players} show={isUtilityLevel} />
