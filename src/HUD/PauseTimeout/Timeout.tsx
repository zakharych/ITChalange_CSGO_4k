...
export default class Timeout extends React.Component<IProps> {
    render() {
        ...
        return (
            <div id={`timeout`} className={`${time && time > 2 && phase && (phase.phase === "timeout_t" || phase.phase === "timeout_ct") ? 'show' : 'hide'} ${phase && (phase.phase === "timeout_t" || phase.phase === "timeout_ct") ? phase.phase.substr(8): ''}`}> { team.name } TIMEOUT
            </div>
        );
    }
}
