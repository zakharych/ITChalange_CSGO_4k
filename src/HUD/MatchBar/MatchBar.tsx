...
export default class TeamBox extends React.Component<IProps, IState> {
  ...
  render() {
    ...
    const isPause = phase && (phase.phase === "paused");
      ...
      <>
        <div id={`matchbar`}>
          ...
          <div id="timer" className={bo === 0 ? 'no-bo' : ''}>
            <div id={`round_timer_text`} className={isPlanted || isPause ? "hide":""}>{time}</div>
            <div id="round_now" className={isPlanted || isPause ? "hide":""}>{this.getRoundLabel()}</div>
