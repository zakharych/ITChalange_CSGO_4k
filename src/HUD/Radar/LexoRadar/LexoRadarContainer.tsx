...
const calculateDirection = (player: Player) => {
  if (directions[player.steamid] && !player.state.health)
    return directions[player.steamid];

  const [forwardV1, forwardV2] = player.forward;
  let direction = 0;

  const [axisA, axisB] = [Math.asin(forwardV1), Math.acos(forwardV2)].map(
...
class App extends React.Component<IProps> {
  ...
  parseGrenadePosition = (grenade: ExtendedGrenade, config: ScaleConfig) => {
    ...
    return this.parsePosition(
      grenade.position.split(", ").map((pos) => Number(pos)),
      size,
      config
    );
  };
  getGrenadePosition = (grenade: ExtendedGrenade, config: ScaleConfig) => {
    ...
    const positions = grenadeData
      .map((grenadeEntry) => this.parseGrenadePosition(grenadeEntry, config))
      .filter((posData) => posData !== null) as number[][];
  ...
  getPosition = (player: Player, mapConfig: ScaleConfig) => {
    const playerData = playersStates
      .slice(0, 5)
      .map(
        (players) => players.filter((pl) => pl.steamid === player.steamid)[0]
      )
      .filter((pl) => !!pl);
    if (playerData.length === 0) return [0, 0];
    const positions = playerData.map((playerEntry) =>
      this.parsePosition(playerEntry.position, config.playerSize, mapConfig)
    );
    const entryAmount = positions.length;
    let x = 0;
    let y = 0;
  ...
  mapPlayer =
    (active: Player | null) =>
    (player: Player): RadarPlayerObject | RadarPlayerObject[] | null => {
      if (!(this.props.mapName in maps)) {
        return null;
      }
      ...
      const playerObject: RadarPlayerObject = {
        ...
        hasBomb: !!Object.values(player.weapons).find(
  ...
  mapGrenade = (extGrenade: ExtendedGrenade) => {
      ...
      const mapFlame = (id: string) => {
        if ("config" in map) {
          return {
            position: this.parsePosition(
              extGrenade.flames[id].split(", ").map((pos) => Number(pos)),
              12,
              map.config
            ),
        ...
        return map.configs.map((config) => ({
          ...
          visible: config.isVisible(
            extGrenade.flames[id].split(", ").map(Number)[2]
          ),
          position: this.parsePosition(
            extGrenade.flames[id].split(", ").map((pos) => Number(pos)),
            12,
    ...
    if ("config" in map) {
      ...
      if (extGrenade.type === "smoke") {
        ...
        extGrenade.type === "flashbang" ||
        extGrenade.type === "frag"
      ) {
    ...
    return map.configs
      .map((config) => {
        ...
        const grenadeObject: RadarGrenadeObject = {
          ...
          visible: config.isVisible(
            extGrenade.position.split(", ").map(Number)[2]
          ),
        };
        if (extGrenade.type === "smoke") {
          if (extGrenade.effecttime !== "0.0") {
  ...
  render() {
    const players: RadarPlayerObject[] = this.props.players
    ...
    const currentGrenades = Object.keys(
      this.props.grenades as { [key: string]: Grenade }
    ).map((grenadeId) => ({
    ...
    const size = this.props.size || 750;
